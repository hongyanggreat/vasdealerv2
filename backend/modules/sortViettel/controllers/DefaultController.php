...
namespace backend\modules\sortViettel\controllers;

use yii;
use yii\web\Controller;
use backend\modules\sortViettel\models\PostsViettel;
use backend\modules\sortViettel\models\TermRelationshipsViettel;
use backend\modules\sortViettel\models\TermTaxonomyViettel;
/**
 * Default controller for the `sortViettel` module
 */
class DefaultController extends Controller
{
    ...
    public function actionListPostByCate(){
        ...
        $module    = Yii::$app->controller->module->id;
         $dataPostsViettel = Yii::$app->request->post();
         if(isset($dataPostsViettel['idCate']) && $dataPostsViettel['idCate']){
                $idCate = $dataPostsViettel['idCate'];
                $postInCates = TermRelationshipsViettel::find()
    ...
    public function actionIndex()
    {
        ...
        if(Yii::$app->request->post()){
            $dataPostsViettel = Yii::$app->request->post();
            $idCate    = $dataPostsViettel['cate'];
            $keyRemove = ['cate','_csrf','dataJson','phantach'];
            // $dataPostsViettel = array_diff($dataPostsViettel, ['cate','_csrf']);
            $dataPostsViettel = array_diff_key($dataPostsViettel, array_flip($keyRemove));
            $arrPostActive = [];
            // echo '<pre>';
            $position = 1;
            foreach ($dataPostsViettel as $idPostAct => $dataPost) {
        ...
        $cates = TermTaxonomyViettel::find()
    ...
    {
        if (($model = PostsViettel::findOne($id)) !== null) {
            return $model;
        } else {
