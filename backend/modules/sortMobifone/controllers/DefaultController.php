...
namespace backend\modules\sortMobifone\controllers;

use yii;
use yii\web\Controller;
use backend\modules\sortMobifone\models\PostsMobi;
use backend\modules\sortMobifone\models\TermRelationshipsMobi;
use backend\modules\sortMobifone\models\TermTaxonomyMobi;
/**
 * Default controller for the `sortMobifone` module
 */
class DefaultController extends Controller
{
    ...
    public function actionListPostByCate(){
        ...
        $module    = Yii::$app->controller->module->id;
         $dataPostsMobi = Yii::$app->request->post();
         if(isset($dataPostsMobi['idCate']) && $dataPostsMobi['idCate']){
                $idCate = $dataPostsMobi['idCate'];
                $postInCates = TermRelationshipsMobi::find()
    ...
    public function actionIndex()
    {
        ...
        if(Yii::$app->request->post()){
            $dataPostsMobi = Yii::$app->request->post();
            $idCate    = $dataPostsMobi['cate'];
            $keyRemove = ['cate','_csrf','dataJson','phantach'];
            // $dataPostsMobi = array_diff($dataPostsMobi, ['cate','_csrf']);
            $dataPostsMobi = array_diff_key($dataPostsMobi, array_flip($keyRemove));
            $arrPostActive = [];
            // echo '<pre>';
            $position = 1;
            foreach ($dataPostsMobi as $idPostAct => $dataPost) {
    ...
    {
        if (($model = PostsMobi::findOne($id)) !== null) {
            return $model;
        } else {
