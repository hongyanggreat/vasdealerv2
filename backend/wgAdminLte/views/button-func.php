	...
	$module = Yii::$app->controller->module->id;
		...
		if(in_array('view', $arrActions)){
			if($dataProvider['task'] == "ButtonControl2"){
				...
				$template .= Html::a('<i class="fa fa-eye""></i>', $linkView, [
                                              'title' => Yii::t('app', 'Xem'),
                                              'class' => 'marginAction',
		...
		if(in_array('update', $arrActions)){
			...
			if($dataProvider['task'] == "ButtonControl2"){
				if($linkUpdate){
					$template .= Html::a('<i class="fa fa-edit"></i>', $linkUpdate, [
	                                              'title' => Yii::t('app', 'Sửa'),
	                                              'class' => 'marginAction',
	                                              'style' => 'margin-left:10px;',
	                                  ]);
		...
		if(in_array('delete', $arrActions)){
			...
			if($dataProvider['task'] == "ButtonControl2"){
				if($linkDelete){
					$template .=  Html::a(' <i class="fa fa-trash"></i>', null, [
                              'data' => [
					...
					/*$template .= Html::a('<i class="fa fa-trash"></i>', $linkDelete, [
	                                              'title' => Yii::t('app', 'Xóa'),
	                                              'class' => 'marginAction',
